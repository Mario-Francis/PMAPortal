...
$(() => {
        ...
        try {
            ...
            if (validateForm(form)) {
                ...
                if (name == '') {
                    ...
                    btn.html('<i class="fa fa-circle-notch fa-spin"></i> Adding pet...');
                    let url = $base + 'pets/AddPet';
                    let data = {
                        name,
    ...
    $(document).on('click', '.edit', async (e) => {
        ...
        let loader = bootLoaderDialog('Fetching pet...');
        let pet = null;
        try {
        ...
        try {
            ...
            if (validateForm(form)) {
                ...
                if (name == '') {
                    ...
                    $('fieldset').prop('disabled', true);
                    btn.html('<i class="fa fa-circle-notch fa-spin"></i> Updating pet...');
                    let url = $base + 'pets/UpdatePet';
                    let data = {
                        id: uid,
                        name,
                        __RequestVerificationToken
    ...
    $(document).on('click', '.delete', async (e) => {
        let loader;
        let uid = $(e.currentTarget).attr('uid');
        bootConfirm('Are you sure you want to delete this pet?', {
            title: 'Confirm Action', size: 'small', callback: async (res) => {
                if (res) {
                    try {
                        loader = bootLoaderDialog('Deleting pet...');
                        let message = await deletePet(uid);
                        loader.hide();
    ...
    var promise = new Promise((resolve, reject) => {
        try {
            if (id == undefined || id == '' || id == 0) {
                ...
                let url = $base + 'pets/DeletePet/' + id;
                $.ajax({
                    type: 'GET',
                    url: url,
                    success: (response) => {
