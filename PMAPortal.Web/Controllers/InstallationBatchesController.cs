...
{
    [Route("[controller]")]
    public class InstallationBatchesController : Controller
    {
        private readonly IInstallationBatchService batchService;
        private readonly IOptionsSnapshot<AppSettings> appSettingsDelegate;
        private readonly ILoggerService<BatchesController> logger;

        public InstallationBatchesController(
            IInstallationBatchService batchService,
        ...
        public IActionResult BatchesDataTable()
        {
            ...
            var batches = batchService.GetBatches().Select(b => BatchVM.FromInstallationBatch(b, clientTimeOffset));
        ...
        public async Task<IActionResult> UploadBatch(IFormFile file, DateTimeOffset? dateShared)
        {
            try
            {
                await batchService.AddInstallationBatch(file, dateShared);
                return Ok(new { IsSuccess = true, Message = "Batch file uploaded and read succeessfully", ErrorItems = new string[] { } });
            }
            catch (AppException ex)
            {
                return StatusCode(400, new { IsSuccess = false, Message = ex.Message, ErrorItems = ex.ErrorItems, ErrorDetail = ex.GetErrorDetails() });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetBatch(long? id)
        {
            try
            {
                ...
                else
                {
                    ...
                    return Ok(new { IsSuccess = true, Message = "Batch retrieved succeessfully", Data = BatchVM.FromInstallationBatch(batch) });
                }
            }
            catch (AppException ex)
            {
        ...
        public async Task<IActionResult> BatchCustomers(long? batchId)
        {
            ...
            return View(BatchVM.FromInstallationBatch(batch));
        }
        ...
        public async Task<IActionResult> BatchCustomersDataTable(long? batchId)
        {
            ...
            var batch = await batchService.GetBatch(batchId.Value);
            var customers = batch.InstallationBatchItems.Select(c => CustomerVM.FromCustomer(c.Customer, clientTimeOffset));
